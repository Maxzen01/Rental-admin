...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $uploadDir = 'uploads/'; // Primary directory
    $secondaryDir = 'C:\\xampp\\htdocs\\Rental\\user\\apartments\\uploads\\'; // Secondary directory
    $adminDir = 'C:\\xampp\\htdocs\\Rental\\admin\\uploads\\'; // Admin directory
    $userDir = 'C:\\xampp\\htdocs\\Rental\\user\\uploads\\'; // user directory
    ...
    if (isset($_FILES['product-main-photo']) && $_FILES['product-main-photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($fileTmpPath, $mainPhotoPath)) {
            // Copy the main photo to the secondary and admin directories
            copy($mainPhotoPath, $secondaryDir . $newFileName);
            copy($mainPhotoPath, $adminDir . $newFileName);
            copy($mainPhotoPath, $userDir . $newFileName);
        } else {
        ...
        foreach ($_FILES['product-several-photos']['tmp_name'] as $key => $tmpName) {
            if ($_FILES['product-several-photos']['error'][$key] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($tmpName, $filePath)) {
                    // Copy the photo to the secondary and admin directories
                    copy($filePath, $secondaryDir . $newFileName);
                    copy($filePath, $adminDir . $newFileName);
                    copy($filePath, $userDir . $newFileName);
  ...
  <div class="container">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
      ...
      <select id="product-category" name="product-category" required>
        ...
        <option value="appartment2">Apartments</option>
